...
MyObjLoader::MyObjLoader():count(0)
{
}
...
bool MyObjLoader::LoadObjFromFileWithAABB(const char* path,std::vector<float>* vertices,
	...
	if (objfile == NULL) {
		printf("MyObjLoader : Impossible to open the file !\n");
		return false;
	}
...
bool MyObjLoader::LoadObjFromFile(const char* path,std::vector<float>* vertices,
		std::vector<float>* uvs,std::vector<float>* normals) {
	char lineHeader[1000];
	std::vector<float> temp_vertices;
	std::vector<float> temp_uvs;
