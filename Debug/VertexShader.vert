...
void main () {
	...
	vec3 vertex_position_cameraspace = ( V * MM * vec4(vertex_position_modelspace,1)).xyz;
	...
	Normal_cameraspace = ( V * MM * vec4(vertex_normal_modelspace,0)).xyz; // Only correct if ModelMatrix does not scale the model ! Use its inverse transpose if not.
